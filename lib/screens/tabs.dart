...
import 'package:icare/screens/pharmacy_fulfillment_dashboard.dart';
...
class _TabsScreenState extends ConsumerState<TabsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (isPharmacyRole(role)) {
      if (currentIndex == 0) {
        activePage = const PharmacyFulfillmentDashboard();
    ...
    } else if (isAdminRole(role) || isSuperAdminRole(role)) {
      if (currentIndex == 0) {
        activePage = const AdminDashboard();
      } else if (currentIndex == 1) {
        activePage = const ClinicalAuditScreen();
      } else if (currentIndex == 2) {
        activePage = const ReferralsScreen();
      } else if (currentIndex == 3) {
        activePage = ProfileScreen();
      }
    } else if (isSecurityRole(role)) {
      if (currentIndex == 0) {
        activePage = const SecurityConsoleScreen();
      } else if (currentIndex == 1) {
        activePage = const ClinicalAuditScreen();
      } else if (currentIndex == 2) {
        activePage = const ReferralsScreen();
...
class _WebSidebar extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final items = [
      ...
      _SidebarItem(
          icon: tabIconForRole(role),
          label: tabLabelForRole(role),
          index: 1),
              ...
              child: Column(
                children: [
                  ...
                  GestureDetector(
                    onTap: () {
                      Navigator.of(context).push(
                        MaterialPageRoute(builder: (ctx) => isPatientRole(role) ? LabReportsScreen() : LabBookingsManagement()),
                      );
                    },
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              quickActionSecondaryLabel(role),
                              style: TextStyle(
                                color: Colors.white,
                                fontSize: 13,
            ...
            child: ListView(
              ...
              children: [
                ...
                if (isPatientRole(role)) ...[
                  ...
                  _buildExtraNavItem(context, Icons.biotech_rounded, isPatientRole(role) ? 'Lab Results & Reports' : 'Management Dashboard', () {
                    Navigator.of(context).push(
                      MaterialPageRoute(builder: (ctx) => isPatientRole(role) ? LabReportsScreen() : LabBookingsManagement()),
                    );
                  }),
                  ...
                  _buildExtraNavItem(context, Icons.school_rounded, coursesLabelForRole(role), () {
                    Navigator.of(context).push(
