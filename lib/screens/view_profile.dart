...
class ViewProfile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final label = isPatientRole(role)
        ? 'Total Appointments'
        : isLabRole(role)
            ? 'Active Requests'
            : isPharmacyRole(role)
        ...
        child: Center(
          child: Column(
            children: [
              ...
              if (!isInstructorRole(role) && !isStudentRole(role)) ...[
                Row(
                  ...
                  children: [
                    ...
                    CustomRecordCard(
                      ...
                      number: isLabRole(role) ? "32" : "4.9",
                      label: isLabRole(role)
                          ? "Completed Reports"
                          : "Ratings",
                    ),
                  ],
              ...
              if (isStudentRole(role) && qualification.isNotEmpty)
                infoRowTile(
                  iconPath: ImagePaths.certificate,
                  infoText: qualification,
                ),
              if (isStudentRole(role) && educationLevel.isNotEmpty)
                infoRowTile(
                  iconPath: ImagePaths.certificate,
                        ...
                        children: [
                          Expanded(
                            child: _buildWebStatCard(
                              label: label,
                              value: "150",
                              icon: isLabRole(role) ? Icons.science_rounded : Icons.people_alt_rounded,
                          ...
                          Expanded(
                            child: _buildWebStatCard(
                              label: isLabRole(role) ? "Completed Reports" : "Average Rating",
                              value: isLabRole(role) ? "32" : "4.9",
                              icon: isLabRole(role) ? Icons.check_circle_rounded : Icons.star_rounded,
